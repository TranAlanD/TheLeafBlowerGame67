...
string path = Application.persistentDataPath + "/" + saveSettings.fileName;

if (File.Exists(path))
{
    // Load settings normally
    saveSettings.LoadGameSettings(File.ReadAllText(path));
}
else
{
    // File missing → create empty/default settings file
    Debug.LogWarning("Settings file not found. Creating a new one...");

    // Create default JSON (depends on your system)
    string defaultJson = "{}"; // temporary safe fallback

    File.WriteAllText(path, defaultJson);

    saveSettings.LoadGameSettings(defaultJson);
}
            try
            {
